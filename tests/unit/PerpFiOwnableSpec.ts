...
import { PerpFiOwnableUpgradeFakeContractFakeInstance } from "../../types/truffle";

const PerpFiOwnableUpgradeFake = artifacts.require(
  "PerpFiOwnableUpgradeFake"
) as PerpFiOwnableUpgradeFakeContract;
...
describe("PerpFiOwnableUpgrade UT", () => {
  ...
  beforeEach(async () => {
    ...
    alice = addresses[1];
    perpFiOwnable =
      (await PerpFiOwnableUpgradeFake.new()) as PerpFiOwnableUpgradeFakeInstance;
  ...
  it("force error set current owner", async () => {
    await expectRevert(
      perpFiOwnable.setOwner(admin),
      "PerpFiOwnableUpgrade: same as original"
    );
  });
