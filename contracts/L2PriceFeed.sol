...
contract L2PriceFeed is IPriceFeed, BlockContext {
    using SafeMath for uint256;

    modifier onlyBridge() {
        require(msg.sender == ambBridge, "!ambBridge");
        _;
    }
    ...
    // FUNCTIONS
    //
    function initialize(address _ambBridge, address _rootBridge) public  {
        ambBridge = _ambBridge;
        rootBridge = _rootBridge;
    }

    function addAggregator(bytes32 _priceFeedKey) external {
    ...
    function setRootBridge(address _rootBridge) external {
